...
#img = cv2.resize(img ,(int(img.shape[2]/5),int(img.shape[0]/5)))
...
#Calculati%ng number of pixels with shade of white(p) to check if exclusion of these pixels is required or not (if more than a fixed %) in order to differentiate the white background or white patches in image caused by flash, if present.
p = 0 
...
if(per == 0):
    print("The leaf is clean")
elif((per>=1) and (per<=10)):
    print("Mild, Spray with Fungicides")
elif((per>=11) and(per<=50)):
    print("Moderate, Spray with Fungicides.")
elif((per>=51)  and (per<=75)):
    print("Tending to severe, Uproot or Spray.")
else:
    print("Severe, Uproot")
